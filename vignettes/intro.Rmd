...
__StatComp__ is a simple R package developed to compare the performance of R and R++ (implemented through the R package _Rcpp_) for the 'Statistical Computing' course. Two functions are considered, namely, _sse_ (Use stratified sampling to estimate the integration.) and _xt_ (estimate the number of events in possion process). For each function, both R and Rcpp versions are produced. Namely _sseR_ and  _xtR_ for R and _sseC_ and _xtC_ for C++.
